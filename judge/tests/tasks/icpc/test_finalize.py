...
from judge.tasks.icpc.testoutput import TestCaseOutput, serialize_outputs
...
class TestFinalizeTask (django.test.TransactionTestCase):
    ...
    def test_finalize_does_not_exit (self):
        with self.assertRaises(Submission.DoesNotExist):
            finalize_task(
                serialize_outputs([ TestCaseOutput(0, TestCaseVerdict.ACCEPTED) ]),
                SubmissionInformation( self.pk + 1, "", "", LanguageKind.PYTHON ).serialize()
            )
    def test_finalize_accepted (self):
        finalize_task(
            ...
            SubmissionInformation( self.pk, "", "", LanguageKind.PYTHON ).serialize()
        )
